...
- [open] ISSUE-001 <title>
  ...
  - Why open: <what blocks closing it>
  - Resolution candidate (optional): <if you have one>

- [closed] ISSUE-002 <title>
  - Discovered: <when/where>
  - Why open: <what blocks closing it>
...
**status 4 态语义**：
- `pending` 还没开始
- `in_progress` 正在做（同一时间通常只有 1-2 个，便于 compact 后接手）
- `done` 完成且 Verify 通过
- `cancelled` 决定不做了 — **必须有 Notes 说明原因**，避免后面的 agent 重新判断要不要做
...
- Open Issue 被解决 → 状态改 closed 并简述结果
...
- **声称完成前**：核对所有 Tasks 都 done、所有 Open Issues 都 closed
...
任务真正完成后（所有 Tasks done、所有 Open Issues closed、verify 全部通过），处理方式：
